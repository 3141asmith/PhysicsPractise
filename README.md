...
Completing a challenge in either course unlocks an animated present. Opening it awards one of three equally weighted prizes: 10 course coins, one pet treat, or one starlight scarf. Each round has a persisted UUID (including server-backed A Level rounds); the course/account browser wallet stores the claimed result under that ID. Repeat clicks and reloads cannot award it again. New rounds have new IDs. Treats can be fed in the pet room for a celebration and a treats-given count; they do not change EXP or require health upkeep. Scarves can be equipped and removed and remain independent of the site theme. Duplicate scarf prizes are retained in inventory. Prize balances remain browser-local, like existing coins. Run `npm run test:prizes` for all outcomes, persistence, duplicate-claim protection, feeding and equipping in both courses.
