const NotesView=(()=>{
  ...
  function poster(topic,title){
    const files={0:'measurements',1:'particles',2:'waves',3:'mechanics',4:'electricity',5:'further-mechanics',6:'fields',7:'nuclear',8:'astrophysics',13:'practical-skills'};
    if(!files[topic])return '';
    const url='assets/alevel-posters/'+files[topic]+'.png';
    return '<figure class="gcse-topic-poster alevel-topic-poster"><figcaption><h2>'+esc(title)+' — revision poster</h2><p><a href="'+url+'" target="_blank" rel="noopener">Open full-size poster</a></p></figcaption><a href="'+url+'" target="_blank" rel="noopener" aria-label="Open '+esc(title)+' revision poster full size"><img src="'+url+'" alt="'+esc(title)+' revision poster with illustrated explanations, equations and key terms. Read the notes above for text explanations." width="1448" height="1086" loading="lazy" decoding="async"></a></figure>';
  }
  function topic(note,title,topic,returnQuestion){
    ...
    }).join('')+poster(topic,title)+'</div><details class="notes-source"><summary>Sources and further reading</summary><p>'+esc(note.source)+'</p></details>';
  }
