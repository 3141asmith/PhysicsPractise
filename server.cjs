...
function createApp({dataDir=path.join(__dirname,'data'),secureCookies=process.env.SECURE_COOKIES==='1',microsoft=readMicrosoftConfig(),microsoftProvider}={}) {
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    try {
      ...
      const allowed=['index.html','a-level.html','home.js','rewards.js','combined-science.html','gcse.html','gcse.js','courses.css','style.css','app.js','school.js','math-format.js','notes.js','notes-view.js','challenge.js','questions.js','extra-calculations.js','extra-explanations.js','practical-questions.js','static-bank.js'];
