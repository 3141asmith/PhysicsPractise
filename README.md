...
The pet room lets students save a name (up to 24 characters), select a starter and choose an evolution branch at 150 EXP. Spark has Solar/Storm branches, Ripple has Reef/Abyss branches, and Pebble has Crystal/Grove branches. Each branch has four evolved forms after the two starter forms and distinct artwork. The first saved name and starter are free and then locked. The shop sells rename tokens for 100 coins and re-class tokens for 500 coins; tokens are consumed only when saving an actual change. The first branch choice at 150 EXP is free, with subsequent branch changes requiring a re-class token. Changes preserve EXP. Profiles and tokens are saved inside the course/account wallet under its existing transaction lock and sync across tabs. Existing named pets are also locked.

Evolution thresholds are 0, 50, 150, 350, 750 and 1550 EXP for every branch. Thresholds follow `50 × (2^stage − 1)`. Levels continue after the final evolution, with level L starting at `25 × (L − 1)^2` EXP. Health stays at 100 without absence penalties. SVG creatures have species/branch colours independent of the site palette and light/dark mode. They wander and bounce in their habitat, respecting reduced-motion settings. Run `npm run test:pet` for browser coverage of rewards, evolution, naming, all starter/branch choices, persistence, cross-tab/course isolation, token purchases and responsive display.

Pet Settings includes an **Animate my pet** checkbox. With no saved choice, it follows the device reduced-motion preference; an explicit choice overrides that preference, persists in this browser and syncs across tabs. The pet room shows full movement; the small widget has occasional subtle motion and a short jumping celebration on earned level/evolution milestones. Wallet refreshes preserve the SVG animation clock. The temporary free-change checkbox is disabled. Previously saved free-edit preferences are cleared when the wallet loads, restoring earned evolution and removing unowned equipped clothing. EXP, tokens, names and owned items are preserved; normal token and ownership rules apply.

Browser numerical questions use a saved random seed to shuffle the authored, physically bounded variants for each calculation family. A Level legacy fixed calculations use generated equivalents from the same topic and difficulty. Question IDs and numbers remain stable across reloads, preserving marking and preventing repeat rewards. The first update clears obsolete numerical drafts and active challenges while retaining mastery, coins and EXP. Server accounts continue using their existing student seed. The Show solution action is temporarily disabled in both courses; correct-answer feedback and written-answer self-assessment remain available. Run `npm run test:randomised` to check different browser seeds, stable reloads, grading and disabled solution actions.
...
The A Level bank includes 12 additional Challenge-level diagram questions (six calculation/written pairs): velocity-time areas, oblique force resolution, nonlinear force-extension energy, cell characteristics, logarithmic capacitor discharge and pendulum length-offset analysis. Diagrams include accessible descriptions and display in practice and challenge views. Values vary with the bank seed; IDs use the `diagram-` prefix. Run `node tests/diagram-browser-tests.cjs` for their focused checks.
...
Completing a challenge in either course unlocks an animated present. Opening it awards one of three equally weighted prizes: 10 course coins, one pet treat, or one wearable cosmetic. Cosmetic prizes choose uniformly from the starlight scarf and 52 clothing designs (13 hats, 13 shoes, 13 jackets and 13 glasses). Each round has a persisted UUID (including server-backed A Level rounds); the course/account browser wallet stores the claimed result under that ID. Repeat clicks and reloads cannot award it again. New rounds have new IDs. Treats can be fed in the pet room for a celebration and a treats-given count; they do not change EXP or require health upkeep. The pet wardrobe lets students equip one owned item in each clothing slot alongside their scarf, preview all designs, and remove clothing. Outfits appear on eggs and every evolution, persist per course/account, and remain independent of the site theme. Existing scarves are preserved. Duplicate cosmetic prizes are retained in inventory. Run `npm run test:cosmetics` to verify every design, ownership checks, persistence and course isolation. Prize balances remain browser-local, like existing coins. Run `npm run test:prizes` for all outcomes, persistence, duplicate-claim protection, feeding and equipping in both courses.

The temporary challenge autocomplete button and API action have been removed. Pet clothing now lives in a separate **Wardrobe** panel opened beside **Settings**. The temporary free-edit checkbox also enables all wardrobe designs and the scarf without granting inventory or spending tokens. Turning it off removes unowned equipped clothing and preserves earned items.

Pet hatch dates are saved in each course wallet as `petHatchedAt` on the first correct completion. The large pet screen shows the hatch date and age in days, completed calendar months or years. Existing hatched pets without a recorded date show an unknown age; cosmetic changes and renaming do not reset the date.
