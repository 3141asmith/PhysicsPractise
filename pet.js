...
const ForgePet=(()=>{
  ...
  function renderDialog(){
    ...
    dialog.innerHTML=`<div class="pet-room-header"><div><p class="section-label">${course} COMPANION</p><h2 id="pet-title">${esc(s.profile.name||stages[s.stage])}</h2></div>${locked?`<button class="secondary" id="pet-settings" type="button" aria-expanded="${settingsOpen}" aria-controls="pet-customise">Settings</button>`:''}<button class="secondary" id="pet-close" type="button" autofocus>Close</button></div>
      ...
      <section class="pet-inventory"><h3>Challenge prizes</h3><p>${s.food} pet treats &middot; ${s.scarves} starlight scarves</p><button id="pet-feed" class="secondary" type="button" ${s.food>0?'':'disabled'}>Feed a treat</button> <button id="pet-equip" class="secondary" type="button" ${s.scarves>0?'':'disabled'}>${s.profile.scarf?'Remove scarf':'Wear scarf'}</button><p class="answer-help">Treats given: ${s.fed}. Mood: ${s.fed?'Delighted':'Curious'}. Treats are for enjoyment and do not change EXP.</p><p id="pet-item-status" role="status"></p><h3>Wardrobe</h3><p class="answer-help">Win cosmetics in challenge presents. Wear one item per slot, alongside your scarf. Colours stay the same in every site theme.</p><div class="pet-wardrobe">${Object.entries(PetCosmetics.slots).map(([slot,label])=>`<label>${label}<select data-outfit="${slot}"><option value="">None</option>${PetCosmetics.items.filter(item=>item.slot===slot).map(item=>`<option value="${item.id}" ${s.profile.outfit[slot]===item.id?'selected':''} ${s.cosmetics[item.id]>0?'':'disabled'}>${item.name} (${s.cosmetics[item.id]||0} owned)</option>`).join('')}</select></label>`).join('')}</div><details class="pet-collection"><summary>View all cosmetic designs</summary><div>${PetCosmetics.items.filter(item=>item.art).map(item=>`<figure>${PetCosmetics.preview(item)}<figcaption>${item.name}<small>${s.cosmetics[item.id]||0} owned</small></figcaption></figure>`).join('')}</div></details></section>
  ...
  function sync(data,course){
    ...
    for(const slot of Object.keys(PetCosmetics.slots)){const id=data.petOutfit?.[slot];if(PetCosmetics.get(id)?.slot===slot&&data.petCosmetics?.[id]>0)current.profile.outfit[slot]=id;}
