...
const ForgePet=(()=>{
  ...
  let current=null,widget,dialog,settingsOpen=false,wardrobeOpen=false,celebrationTimer;
  ...
  function creature(stage,p=profile()){
    ...
    if(stage<0){
      ...
      return '<svg viewBox="0 0 240 200" data-pet-family="'+p.starter+'" data-pet-branch="'+p.branch+'" data-pet-stage="0" data-pet-egg="true" aria-hidden="true" focusable="false"><ellipse class="pet-shadow" cx="120" cy="179" rx="51" ry="8"/><g class="pet-bob"><path class="pet-body" d="M72 140C72 105 100 49 120 49S168 105 168 140Q168 178 120 178Q72 178 72 140Z"/><path d="M87 104Q96 75 110 68" fill="none" stroke="#fff" stroke-width="7" opacity=".45" stroke-linecap="round"/>'+decoration+clothing('jacket')+scarf+clothing('shoes')+clothing('glasses')+clothing('hat')+'</g></svg>';
    }
    ...
    return '<svg viewBox="0 0 240 200" data-pet-family="'+p.starter+'" data-pet-branch="'+p.branch+'" data-pet-stage="'+stage+'" aria-hidden="true" focusable="false">'+
      ...
      (p.starter==='spark'?'<ellipse class="pet-belly" cx="122" cy="143" rx="27" ry="24"/>':'')+front+clothing('jacket')+
      '<g class="pet-gaze"><g class="pet-eyes"><ellipse cx="95" cy="103" rx="'+(p.starter==='pebble'?7:8)+'" ry="'+(p.starter==='pebble'?8:12)+'"/><ellipse cx="145" cy="103" rx="8" ry="'+(p.starter==='pebble'?8:12)+'"/><circle class="pet-glint" cx="98" cy="99" r="3"/><circle class="pet-glint" cx="148" cy="99" r="3"/></g></g>'+path('M110 123Q120 133 130 123','pet-smile pet-mouth')+scarf+feet+clothing('shoes')+clothing('glasses')+clothing('hat')+'</g></g></svg>';
  }
  ...
  function mount(){
    ...
    widget.onclick=()=>{settingsOpen=false;wardrobeOpen=false;renderDialog();dialog.showModal();widget.dataset.roomOpen='true';};
  ...
  function renderDialog(){
    ...
    const stages=journey(s.profile),previousArt=dialog.querySelector('.pet-habitat svg'),artKey=[s.stage,s.profile.starter,s.profile.branch,s.profile.scarf,JSON.stringify(s.profile.outfit)].join(':');
    const locked=!!s.profile.name,rename= s.freeEdits||!locked||s.renameTokens>0,reclass= s.freeEdits||!locked||s.reclassTokens>0;
    dialog.innerHTML=`<div class="pet-room-header"><div><p class="section-label">${course} COMPANION</p><h2 id="pet-title">${esc(s.profile.name||stages[s.stage])}</h2></div><button class="secondary" id="pet-settings" type="button" aria-expanded="${settingsOpen||!locked&&!wardrobeOpen}" aria-controls="pet-customise">Settings</button><button class="secondary" id="pet-wardrobe-open" type="button" aria-expanded="${wardrobeOpen}" aria-controls="pet-wardrobe-panel">Wardrobe</button><button class="secondary" id="pet-close" type="button" autofocus>Close</button></div>
      ...
      <form id="pet-customise" ${wardrobeOpen||(locked&&!settingsOpen)?'hidden':''}><label for="pet-name">Pet name (up to 24 characters)</label><input id="pet-name" maxlength="24" required ${rename?'':'disabled'} value="${esc(s.profile.name)}" placeholder="Give your companion a name" autocomplete="off">
    ...
    document.getElementById('pet-customise').insertAdjacentHTML('afterbegin',`<label><input id="pet-animation" type="checkbox" ${motionEnabled()?'checked':''}> Animate my pet</label><p class="answer-help">Turn on to enable movement, including when your device prefers reduced motion.</p><label><input id="pet-free-edits" type="checkbox" ${s.freeEdits?'checked':''}> Allow free name, class, evolution and wardrobe changes (temporary)</label><p class="answer-help">No tokens are spent while enabled, and all wardrobe designs are available. Turning this off removes unowned clothing. Earned EXP and owned items are unchanged.</p>`);
    applyMotion();
    ...
    dialog.querySelectorAll('[data-outfit]').forEach(select=>select.onchange=async()=>{try{await Rewards.petItem('outfit',{slot:select.dataset.outfit,id:select.value});document.getElementById('pet-item-status').textContent='Outfit saved.';}catch(error){select.value=current.profile.outfit[select.dataset.outfit]||'';document.getElementById('pet-item-status').textContent=error.message;}});
    const settings=document.getElementById('pet-settings');
    const wardrobe=document.getElementById('pet-wardrobe-open');
    function panels(){document.getElementById('pet-customise').hidden=wardrobeOpen||(!settingsOpen&&locked);document.getElementById('pet-wardrobe-panel').hidden=!wardrobeOpen;settings.setAttribute('aria-expanded',String(!document.getElementById('pet-customise').hidden));wardrobe.setAttribute('aria-expanded',String(wardrobeOpen));}
    settings.onclick=()=>{settingsOpen=document.getElementById('pet-customise').hidden;wardrobeOpen=false;panels();};
    wardrobe.onclick=()=>{wardrobeOpen=!wardrobeOpen;settingsOpen=false;panels();};
