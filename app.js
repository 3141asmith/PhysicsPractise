function startPractice(bank) {
  ...
  function renderList(list){
    $('question-list').innerHTML=list.map(q=>'<button class="question-link '+(selected===q.id?'active':'')+'" data-id="'+q.id+'"><span class="mini-meta"><span class="badge '+q.level+'">'+q.level+'</span><span>'+(saved[q.id]?.mastered?'<span class="mastered-status">Mastered<span class="mastered-tick" aria-hidden="true">&#10003;</span></span>':q.marks+' marks')+'</span></span><strong>'+escapeHtml(q.title)+'</strong><span class="question-preview">'+math(q.prompt)+'</span><small>'+escapeHtml(TOPICS[q.topic])+' &middot; '+(q.type==='written'?'Written response':q.type==='choice'?'Multiple choice':'Calculation')+(saved[q.id]?.hintCount?' &middot; '+saved[q.id].hintCount+' hints used':'')+'</small></button>').join('');
