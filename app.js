function questionTypeBadge(type) {
  const label={numeric:'Numerical',choice:'Multiple choice',written:'Written response'}[type]||'Question';
  return '<span class="badge question-type">'+label+'</span>';
}
function startPractice(bank) {
  ...
  function renderList(list){
    $('question-list').innerHTML=list.map(q=>'<button class="question-link '+(selected===q.id?'active':'')+'" data-id="'+q.id+'"><span class="mini-meta"><span class="question-badges"><span class="badge '+q.level+'">'+q.level+'</span>'+questionTypeBadge(q.type)+'</span><span>'+(saved[q.id]?.mastered?'<span class="mastered-status">Mastered<span class="mastered-tick" aria-hidden="true">&#10003;</span></span>':q.marks+' marks')+'</span></span><strong>'+escapeHtml(q.title)+'</strong><span class="question-preview">'+math(q.prompt)+'</span><small>'+escapeHtml(TOPICS[q.topic])+' &middot; '+(q.type==='written'?'Written response':q.type==='choice'?'Multiple choice':'Calculation')+(saved[q.id]?.hintCount?' &middot; '+saved[q.id].hintCount+' hints used':'')+'</small></button>').join('');
  ...
  function render(){
    ...
    $('question').innerHTML='<article class="question-sheet"><div class="question-top"><span class="question-badges"><span class="badge '+q.level+'">'+q.level+'</span>'+questionTypeBadge(q.type)+'</span><span>'+q.marks+' marks</span></div><h2>'+escapeHtml(q.title)+'</h2><p class="prompt">'+math(q.prompt)+'</p>'+QuestionDiagrams.render(q.diagram)+'<form id="answer-form">'+answer+'<div class="actions"><button type="submit" class="primary">'+(q.type==='written'?'Compare answer':'Check answer')+'</button><button type="button" id="reveal" class="secondary" disabled title="Solutions are temporarily disabled">'+(q.steps?'Hide solution':'View solution')+'</button><button type="button" id="hint" class="secondary">Hint</button></div></form><div id="feedback" aria-live="polite">'+(feedback[q.id]||'')+'</div><section id="hints" aria-live="polite"></section><div id="solution"></div><div class="question-bottom"><a id="question-notes" href="#notes/'+q.topic+'">Read '+escapeHtml(TOPICS[q.topic])+' notes &rarr;</a><div><button id="prev" class="secondary" title="Previous question" '+(!index?'disabled':'')+'>&larr;</button> <button id="next" class="secondary" title="Next question" '+(index===list.length-1?'disabled':'')+'>&rarr;</button></div></div></article>';
