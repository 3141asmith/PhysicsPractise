...
(async()=>{
 ...
 try{
  ...
  for(const gcse of [true,false]){
   ...
   await page.locator('#pet-wardrobe-open').click();await page.locator('#pet-equip').click();await page.locator('.pet-habitat .pet-scarf').waitFor();
   await page.locator('#pet-close').click();await page.reload();await page.locator('#forge-pet .pet-scarf').waitFor();
   await page.locator('#forge-pet').click();await page.locator('#pet-wardrobe-open').click();await page.locator('#pet-equip').click();await page.waitForFunction(()=>!document.querySelector('#forge-pet .pet-scarf'));
   await page.locator('#pet-close').click();
  }
  assert.equal(await page.locator('#challenge-auto-complete').count(),0);
  assert.equal(await page.evaluate(async()=>{try{await School.api('/api/challenge',{action:'auto-complete'});return false;}catch{return true;}}),true);
