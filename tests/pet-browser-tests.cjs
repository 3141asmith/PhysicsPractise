...
(async()=>{
 ...
 try{
  ...
  await eggPage.locator('#pet-free-edits').check();await eggPage.waitForFunction(()=>!document.getElementById('pet-form').disabled);
  await eggPage.locator('#pet-name').fill('Egg preview');await eggPage.locator('#pet-form').selectOption('5');await eggPage.locator('#pet-customise [type=submit]').click();
  await eggPage.waitForFunction(()=>document.querySelector('#forge-pet svg').dataset.petStage==='5');
  assert.ok((await eggPage.locator('#forge-pet').innerText()).includes('Lv 0'));assert.ok((await eggPage.locator('#forge-pet').innerText()).includes('0 EXP'));
  await eggPage.reload();await eggPage.locator('#forge-pet').click();assert.equal(await eggPage.locator('#forge-pet svg').getAttribute('data-pet-stage'),'5');
  await eggPage.locator('#pet-settings').click();await eggPage.locator('#pet-form').selectOption('');await eggPage.locator('#pet-customise [type=submit]').click();
  await eggPage.waitForFunction(()=>document.querySelector('#forge-pet svg').dataset.petEgg==='true');
  await eggPage.locator('#pet-settings').click();await eggPage.locator('#pet-form').selectOption('2');await eggPage.locator('#pet-customise [type=submit]').click();
  await eggPage.waitForFunction(()=>document.querySelector('#forge-pet svg').dataset.petStage==='2');
  await eggPage.locator('#pet-settings').click();await eggPage.locator('#pet-free-edits').uncheck();await eggPage.waitForFunction(()=>document.querySelector('#forge-pet svg').dataset.petEgg==='true');
  await eggPage.close();
