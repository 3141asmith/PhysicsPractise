...
(async()=>{
  ...
  try{
    ...
    await page.waitForFunction(()=>document.getElementById('shop-credits').textContent==='1');
    assert.equal(await page.locator('#coin-balance').innerText(),'0');
    ...
    assert.equal(await page.locator('#shop-credits').textContent(),'0');
    ...
    await page.evaluate(async()=>{
      ...
      if(document.getElementById('shop-credits').textContent!=='1')throw Error('First GCSE hint was charged');
      await Rewards.perform('/api/hints/gcse-test-0',{},reveal,progress);
      if(document.getElementById('shop-credits').textContent!=='0')throw Error('GCSE paid hint was not charged');
    });
