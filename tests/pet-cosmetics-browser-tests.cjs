 ...
 try{
  ...
  for(const item of items){
   const receipt=await page.evaluate(async item=>{
    await School.api('/api/challenge',{action:'restart'});
    const state=await School.api('/api/challenge',{action:'auto-complete'});
  ...
  await page.locator('#forge-pet').click();await page.locator('.pet-collection summary').click();
  ...
  await page.goto(base+'/gcse.html');await page.locator('#forge-pet').click();
